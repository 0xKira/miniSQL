        ...
        if (keyNum < capacity) {
            ...
            if (type == -1) {
                ...
                for (int j = keyNum; j > i; j--)
                    memcpy(currentNode + HEADLEN + j * moduleSize, currentNode + HEADLEN + (j - 1) * moduleSize,
                           moduleSize);
        ...
        *(int *) (buf + root * NODESIZE + 5) = -1;
        ...
        if (*(int *) (node + 9) + *(int *) (nNode + 9) < capacity) {
            ...
            if (!node[0]) {
                ...
                for (i = 0; i < *(int *) (node + 9); i++)
                    memcpy(nNode + HEADLEN + (*(int *) (nNode + 9) + i) * moduleSize, node + HEADLEN + i * moduleSize,
                           moduleSize);
                *(int *) (nNode + 9) += *(int *) (node + 9);
                memcpy(nNode + HEADLEN + *(int *) (nNode + 9) * moduleSize,
                       node + HEADLEN + *(int *) (node + 9) * moduleSize, 4);
            }
            char *parent = new char[NODESIZE];
            ...
            if (!ff) {
                if (!node[0]) {
                    ...
                    saveToBuf(nNode);
                    delete[] nNode;
                    *(int *) (node + HEADLEN + (*(int *) (node + 9) + 1) * moduleSize) = *(int *) (node + HEADLEN +
                                                                                                   *(int *) (node + 9) *
                                                                                                   moduleSize);
                    for (i = 0; i < *(int *) (node + 9); i++)
                ...
                if (!nNode[0]) {
                    ...
                    saveToBuf(node);
                    delete[] node;
                    *(int *) (nNode + HEADLEN + (*(int *) (nNode + 9) + 1) * moduleSize) = *(int *) (nNode + HEADLEN +
                                                                                                     *(int *) (nNode +
                                                                                                               9) *
                                                                                                     moduleSize);
